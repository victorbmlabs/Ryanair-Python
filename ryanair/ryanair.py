...
class Ryanair:
    ...
    def _retryable_query(self, url, params=None) -> Dict:
        self._num_queries += 1
    ...
    def _parse_cheapest_flight(self, flight):
        ...
        return Flight(
            departure_time=datetime.fromisoformat(flight["departureDate"]),
            flight_number=f"{flight['flightNumber'][:2]}{flight['flightNumber'][2:]}",
            price=flight["price"]["value"],
            currency=currency,
            origin=flight["departureAirport"]["iataCode"],
            origin_city=flight["departureAirport"]["name"],
            origin_country=flight["departureAirport"]["countryName"],
            destination=flight["arrivalAirport"]["iataCode"],
            destination_city=flight["arrivalAirport"]["name"],
            destination_country=flight["arrivalAirport"]["countryName"],
        )

    def _parse_cheapest_return_flights_as_trip(self, outbound, inbound) -> Trip:
        ...
        return Trip(
            outbound=outbound,
            inbound=inbound,
            total_price=inbound.price + outbound.price,
        )
